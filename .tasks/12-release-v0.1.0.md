...
- README section "Versioning": semantic versioning policy; metric definitions are versioned independently and a metric version bump is a minor release with a changelog note; support-matrix changes are documented per release.
...
- Post-release: open `after v0.1.0` task files for Git ratcheting (compare each method against its value at a resolved base revision so existing methods may not get worse while new ones meet the normal limit), class coupling, named presets and persistent cache if it was deferred in task 11.
