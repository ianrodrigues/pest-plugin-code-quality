...
A read-only way to see what a policy selected and measured, even when everything passes, plus a versioned machine-readable report.
...
- JSON schema v1 (`schema/quality-report.v1.json`, described in a README section "JSON report"): `schemaVersion`, `generatedAt`, `pest`/`plugin`/`php` versions, `policies[]` with identity, targets, completeness counts, `measurements[]`, `violations[]`, `errors[]`, `truncated: false` always for JSON.
