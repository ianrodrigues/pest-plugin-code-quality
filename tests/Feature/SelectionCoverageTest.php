...
use Rdgs\PestCodeQuality\Selection\VendorTarget;
...
it('errors on a target that resolves entirely under vendor', function (): void {
    $chain = policy(fn () => expect('PhpParser\Node\Expr')->classes()->toHaveMethodComplexityAtMost(10));
...
it('errors instead of warning once strict mode is on', function (): void {
    ...
    expect(fn () => $runner->run(Policy::complexity(99), $targets, $options))
