...
it('exposes methods by symbol and counts them on FileMeasurements', function (): void {
    ...
    expect($measurements)->toHaveCount(2)
        ->and($measurements->bySymbol('A::a')?->line)->toBe(1)
        ->and($measurements->bySymbol('A::missing'))->toBeNull()
        ->and(iterator_to_array($measurements))->toHaveCount(2);
});
